...
uint8_t DHT11_Read(uint8_t *data)
{
    ...
    if (dht11_wait_level(GPIO_PIN_RESET, 100)) return 1;  /* 等变低(响应来了) */
    if (dht11_wait_level(GPIO_PIN_SET,  100)) return 1;   /* 等变高(80us 低结束) */
    if (dht11_wait_level(GPIO_PIN_RESET, 100)) return 1;  /* 等再变低(80us 高结束, 数据要来) */
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        for (j = 0; j < 8; j++)
        {
            ...
            if (dht11_wait_level(GPIO_PIN_SET, 100)) return 1;
            ...
            if (dht11_wait_level(GPIO_PIN_RESET, 100)) return 1;
        }
        data[i] = byte;
    }
